...
export default function Home() {
  ...
  const [yearFrom, setYearFrom] = useState("2008");
  const [yearTo, setYearTo] = useState("2024");
  const [sort, setSort] = useState<SortOption>("endingAsc");
  const [fuelType, setFuelType] = useState<FuelTypeKey | "">("");
  const [transmission, setTransmission] = useState<TransmissionKey | "">("");
  const { t } = useTranslation();

  const fuelOptions = useMemo(
    () => orderedFuelTypes.map((key) => ({ key, label: t(fuelTranslationKey(key)) })),
    [t]
  );

  const transmissionOptions = useMemo(
    () => orderedTransmissions.map((key) => ({ key, label: t(transmissionTranslationKey(key)) })),
    [t]
  );
  ...
  const filteredCars = useMemo(() => {
    ...
    return cars
      .filter((car) => {
        ...
        if (fuelType && normalizedFuel !== fuelType) return false;
        if (fuelType && !normalizedFuel) return false;

        const normalizedTransmission = normalizeTransmission(car.transmission);
        if (transmission && normalizedTransmission !== transmission) return false;
        if (transmission && !normalizedTransmission) return false;
  ...
  }, [cars, searchTerm, yearFrom, yearTo, sort, fuelType, transmission]);
  ...
  return (
    <div className="space-y-16">
      <section className="grid gap-0 overflow-hidden rounded-[32px] bg-white shadow-lg ring-1 ring-neutral-200 md:grid-cols-[1.05fr_1fr]">
        ...
        <div className="px-8 py-10">
          ...
          <form onSubmit={handleSearchSubmit} className="mt-6 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <label htmlFor="fuelType" className="text-sm font-medium text-neutral-600">
                  {t("home.search.fuel")}
                </label>
                <select
                  id="fuelType"
                  value={fuelType}
                  onChange={(event) => setFuelType(event.target.value as FuelTypeKey | "")}
                  className="w-full rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-800 shadow-inner focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-100"
                >
                  <option value="">{t("common.fuel.any")}</option>
              ...
              <div className="space-y-2">
                <label htmlFor="transmission" className="text-sm font-medium text-neutral-600">
                  {t("home.search.transmission")}
                </label>
                <select
                  id="transmission"
                  value={transmission}
                  onChange={(event) => setTransmission(event.target.value as TransmissionKey | "")}
                  className="w-full rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-800 shadow-inner focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-100"
                >
                  <option value="">{t("common.transmission.any")}</option>
      ...
      <section id="jak-kupowac" className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="card space-y-6">
          <h3 className="text-2xl font-semibold text-neutral-900">{t("home.watchlist.title")}</h3>
          <p className="text-sm text-neutral-600">{t("home.watchlist.description")}</p>
          <form className="grid gap-4 sm:grid-cols-[1fr_auto]">
            <input
              type="text"
              placeholder={t("home.watchlist.placeholder")}
              className="w-full rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-800 shadow-inner focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-100"
            />
            <button
              type="submit"
            ...
            >
              {t("home.watchlist.button")}
            </button>
          </form>
          <p className="text-xs text-neutral-400">{t("home.watchlist.consent")}</p>
        </div>
        <div className="card space-y-4" id="kontakt">
          <h3 className="text-2xl font-semibold text-neutral-900">{t("home.transport.title")}</h3>
          <p className="text-sm text-neutral-600">{t("home.transport.description")}</p>
          <a
            href="#kalkulator"
          ...
          >
            {t("home.transport.button")}
          </a>
          <div className="rounded-2xl bg-neutral-50 p-4 text-sm text-neutral-600">
            <p className="font-semibold text-neutral-800">{t("home.transport.helpTitle")}</p>
            <p className="mt-2">{t("home.transport.phone")}</p>
            <p>{t("home.transport.email")}</p>
          </div>
        </div>
      </section>
    </div>
