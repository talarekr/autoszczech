...
export default function CarCard({ car }: CarCardProps) {
  ...
  return (
    <article className="grid overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-neutral-200 transition hover:-translate-y-1 hover:shadow-lg md:grid-cols-[minmax(0,320px),1fr]">
      <div className="relative h-full w-full min-h-[220px] bg-neutral-100 md:min-h-[260px]">
        <div className="relative h-full w-full">
          {thumbnail ? (
            <img
              src={thumbnail}
              alt={`${car.make} ${car.model}`}
              className="aspect-[4/3] h-full w-full object-cover md:absolute md:inset-0 md:h-full"
              loading="lazy"
            />
          ) : (
